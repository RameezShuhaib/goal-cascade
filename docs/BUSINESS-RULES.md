...
- **Create**: title required. When created as a sub-goal, the horizon picker only allows ranks below the parent's; the parent picker only offers goals with a longer horizon. The period defaults to the current one for that horizon.
...
- Every lens except Life is scoped to **one period**, with a chevron each way: Yearly by year, Quarterly by quarter, Monthly by month, Weekly by week. Life is simply all of them. Both chevrons and the picker reach the same range, and there is no cap in either direction. A period holding work is marked in the picker, so a goal set three months out is not invisible until you get there.
...
- A future period is badged `Planning ahead` and nothing more. Work that has not come due is never styled as late.
