...
**`docs/BUSINESS-RULES.md` is unchanged by the web half.** A11 §8.6's sentence and A8's own month-band and
Measure paragraphs already say what this build shipped — *"Below both of those sits **this month**…"*,
*"**Nothing in that month band wears a late label**…"* and the whole Measure section — so
`apps/api/src/api/mcp/business-rules.ts` needed no regeneration, and
`apps/api/tests/mcp/verbatim.test.ts` stays green as the proof.
