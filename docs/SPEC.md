...
- **R-goal-46 (`Repeat last week` — the answer to node proliferation)** ⚠ **AMENDED by A10 — it renders ONCE, at the foot of the Weekly lens's list, and copies the previous week's Weekly goals across EVERY Life line.** `repeatWeek`'s `lifeGoalId` becomes **optional**; absent means all lines, and supplying it narrows to one exactly as before. Q-22 required it per line because the control lived at a group foot; there are no group feet (R-lens-3, deleted), and a per-line variant would need a per-line row, which is a group header by another name. **Q-22's objection to account-wide — *twenty goals in one tap with no review* — is answered by the cap rather than by the parameter**: the copies count against `MAX_WEEKLY_GOALS_PER_WEEK` like any other Weekly goal and an overflowing repeat is refused **whole**, so the blast radius is bounded by the same number that bounds a week. Everything else is unchanged: ordinary new goals, `pulse` reset, no tasks copied, nothing linking a copy to its source, current week or later only, and the no-op toast. ~~modified by the reconciliation pass — it now has a place to be tapped. It lives at the **group foot in the Weekly lens**, beside that group's `+ Weekly goal`, and nowhere else.~~ Q-22's original placement — "from the Life group's own header" — is impossible: the header is entirely one collapse toggle with no separate button (R-lens-19). The group foot is a row that already exists, keeps the action per-line as Q-22 requires, adds no chrome, and leaves the header one gesture — The Weekly lens offers one action, `Repeat last week`, which copies the previous week's Weekly goals **for that Life line** into the selected week as **ordinary new goals**: `title`, `why` and `parentId` carried over, `pulse` reset to `On track`, `periodKey` set to the selected week, new ids, no tasks copied, and **nothing linking a copy to its source**. It is offered only on the current week or a later one (R-goal-36), and is a no-op with a toast when the previous week held nothing.
...
- **R-goal-47 (the planned-ness line, and dormancy's one surface)** ⚠ **AMENDED BY A8’S WEB HALF (`33-measurables-ux` §7.3): when a Monthly goal has NO Weekly goals and its card now renders AT LEAST ONE month task (R-lens-32), the line reads `No weeks yet` rather than `Nothing planned yet` — the original string is a claim about the month that the tasks directly beneath it contradict, and the new one is the same fact narrowed to what the line is actually about. With no weeks AND no tasks the string is unchanged, verbatim. ⚠ It also settles the line's POSITION against A8's own note below: it renders ABOVE the nested task list, in its existing slot between the Life line and the backlog line, because the card's muted register is one run and splitting it around a list would put two orientation lines on either side of the content they orient.** ⚠ **modified by A8 — the line still counts WEEKLY GOALS and nothing else, and it still renders; but `Nothing planned yet` on a month holding three month tasks is a lie, so the empty case splits in two (one new row below), and the line now renders BENEATH the card's task list (R-lens-32)** — A Monthly goal's card, in the Monthly lens and on its detail page, carries **one muted line** stating how the month is broken into weeks. The scope is *Weekly goals whose parent chain reaches this Monthly goal and whose week's **Monday** falls in the viewed month*. ⚠ **A9 — the claim that "one answer serves all three and they can never disagree" was false and is withdrawn.** This scope is a `BETWEEN firstMondayIn … lastMondayIn` range scan and is **unchanged**; R-lens-9's zoom asks a different question (the week you are living in) and keeps its own answer under its own name, `zoomWeekForMonth`; R-task-49's target week asks a third (a week inside this month) and takes `taskWeekForMonth`. What all three share is the **Monday rule**, which is R-goal-33's and is genuinely one rule; what they were wrongly sharing was one *function*, and this scope was the only one of the three the shared function never broke.
...
- **R-lens-3 (grouping by Life goal, at any depth)** ⚠ **DELETED by A10 — the owner reversed the instruction that created it: _"also lets not categorise based on life in any horizon."_ Every lens is a flat list at every horizon (R-lens-5, rewritten); orientation moved onto the card as R-lens-23's `under <Life goal>` line, and the counts moved to the Life lens (R-lens-4, rewritten). `lifeRootId` and `LensResponse.groups` stay on the wire — the Life line each item belongs to is what that line names, and the goal picker still groups by it (R-nav-31) — but grouping is no longer a rendering unit anywhere.** ~~Every lens except Life groups its items under **the Life goal each ultimately belongs to**, resolved by walking `parentId` to the root — any depth, with no assumption that the chain is four long or that the levels are adjacent (R-goal-32). Grouping is **not** filtering: there is no `All` chip and no way to view one Life line alone, because the grouping already answers that. *This was the replacement for the goal-filter pills (R-rm-4); the owner was explicit: "in each lense we donot need a filter on goals instead it will be catogrised by life goals."*~~ **The pills stay deleted (R-rm-4, R-lens-15): what A10 removes is the categorisation, not the filter it replaced. A lens is a flat list with no filter of any kind.**
...
- **R-lens-4 (the per-line open-task count, and its one home)** ⚠ **REWRITTEN by A10 — there is no group header at any horizon (R-lens-3, deleted). The count of open tasks under a Life goal, in the anchoring week, renders on _that Life goal's card in the Life lens_ and nowhere else in any lens. A zero is never rendered. The scope below is unchanged and is why the move costs nothing: R-lens-4 already anchored the count to the CURRENT week everywhere except the Weekly lens, so the number on a Life card is the number the header showed on four of the five lenses — and the count is a fact about a LINE, which on a flat Monthly list three cards from one line would have printed three times.** ⚠ **modified by the reconciliation pass — a zero count is not rendered; the header's shape and collapse are R-lens-19's** ⚠ **modified by A8 — the count is **week tasks only**; a month task is never counted in it, at any lens (R-lens-31)** — Each group header shows the Life goal's title, its pulse dot, and its **open-task count** — the surviving home of the counts the filter pills carried (R-nav-7, owner decision 7). The count is the number of **open tasks under that Life goal visible in the anchoring week**: the **selected** week in the Weekly lens, and the **current** week in every other lens, which have no week of their own.
...
- **R-lens-5 (order in a lens)** ⚠ **REWRITTEN by A10 — a lens body is a FLAT LIST, and the order on the wire is the order on the screen** — A lens body is a **flat list**, ordered by the item's **Life root** (`createdAt` asc, then `id` asc — the same total, stable order as every sibling list, Q-7), then by the **item** (`createdAt` asc, then `id` asc), with **root-less items last** (R-lens-20). Every item at one horizon in one period shares a `periodKey`, so no period tie-break is needed. No list order in a lens depends on storage order, and no lens is manually re-orderable (manual order exists for backlog items and nowhere else, R-backlog-21).
  - **This is character for character the reading order of the previously grouped screen with its headers removed.** That is the whole point: cards from one line stay **adjacent**, so the repeated `under <Life goal>` line reads as a label on a run rather than as noise on every card, and **the same goal is in the same place before and after the change** — muscle memory survives.
  - ⚠ **`LensResponse.items` arrives in this order and the client does not re-sort and does not partition.** The old rule specified order *within* a group and left the client to re-partition the page; with no groups to partition into, the flat total order is **load-bearing on the wire**. The server sorts the page it has just read, after taking `nextCursor`: the cursor is the `(createdAt, id)` pair the underlying query pages on, and re-keying pagination on a walked-up Life root would need a denormalised `life_root_id` column this design refuses (R-lens-27). One page is what one screen renders, which is exactly the scope the client's own partition covered.
  - **Refused:** order by pulse (a list that reorders when you change a dot is unlearnable), by open count (same, worse), alphabetical (meaningless across lines), most-recent-first (the plan is not a feed), and no order guarantee at all (the server already has a total order and must keep it).
...
- **R-lens-13 (the lens switcher)** ⚠ **CLARIFIED by A10, not overturned — its bottom-tab-bar refusal stands verbatim (five lenses in the three-item destination bar leaves no room for capture or Learnings, and the tab bar is a top-level destination switcher, not a zoom), and it does NOT forbid a lens tab strip inside the Goals screen (R-lens-33). What A10 overturns is the extension of that argument to any tab-shaped control anywhere in the product. Its accessibility clause — one tab stop, arrow keys along the axis the list runs, the selection announced rather than merely coloured — binds R-lens-33 in full and is its most literal implementation in the app.** ⚠ **SUPERSEDED by R-lens-17 (reconciliation pass) — there is no persistent switcher; the title is the control; ⚠ its surviving accessibility clause is GENERALISED by R-nav-31 (A6) — a single tab stop, arrow keys along the axis the list runs, and the selection announced rather than merely coloured, at EVERY picker in the product rather than only in the Zoom sheet** — ~~A five-way control — `Life · Yearly · Quarterly · Monthly · Weekly` — in the Goals screen header, above the period control.~~ The refusal it carried survives verbatim and is restated in R-lens-17 and R-nav-23: **it is not a tab and must never become one** — five lenses in a five-item tab bar leaves no room for capture or Learnings, and the tab bar is a top-level destination switcher, not a zoom. Its accessibility requirements also survive into the Zoom sheet: a single tab stop with `←`/`→` between options (the roving-tabindex pattern R-backlog-22 already requires), and the selected lens announced, not merely coloured.
...
- **R-lens-17 (the lens control is a tab strip)** ⚠ **REWRITTEN by A10 on the owner's evidence after daily use: _"i dont need to click on a dropdown to change the lense as it adds friction. instead can we have a tabs in the top where i dont need double clicking to change lense and below that i can change the timestamp as normal."_** — The lens is chosen from a **horizontal tab strip** (`role="tablist"`, R-lens-33) directly below the top-right cluster: the five horizons in horizon order, `Life` first, **one tap** to change. The strip scrolls horizontally and keeps the selected tab in view; **no label is ever shortened, truncated or scaled.** The period row sits below it and carries `‹`, the period title as **text, not a button**, and `›`. **There is no Zoom sheet.** *Supersedes R-lens-13; retires R-lens-22.*
  - ⚠ **What the Zoom sheet carried, and where each part went.** *Change lens* → the strip, at **zero** taps of overhead instead of two. *The exact period each lens would land on, before you commit* → the period row 44px below the tab you just pressed, rendered in the **same frame** (R-lens-30) after a tap that is free and one tap reversible — a preview is only worth its own surface when the commit is expensive, and tabs make it free. *A goal count per lens* → **deleted** (R-lens-22). *`Jump to now`* → already duplicated by the off-now row's `Now ›` (R-lens-21), which renders in exactly the same condition. *`aria-current` on the current lens* → `aria-selected` on the current tab, which is stronger: it is on screen continuously rather than only inside a sheet.
  - ⚠ **The zoom MODEL is untouched.** R-lens-9's clamp, R-lens-18's derived anchor, `zoomTo`, the Life-is-not-a-reset property and the reversibility guarantee all stand and are all still exercised — by the tabs, by `Shift+↑`/`Shift+↓` and by R-nav-32's horizon selector. Only the sheet that displayed them is gone.
  - ⚠ **The title stops being a control**: no `▾` marker, no `aria-label`, no tab stop, no hover, no press state. It is 21px/800 `T.ink`, `nowrap`, ellipsising, exactly as before. **This is what pays for part of the tab row** — row 3 loses its only non-chevron control and one tab stop, so the shell gains five tabs and one stop and loses one, and the focus order gets shorter on any account with more than one Life line. A screen reader hears the period through the live region (R-lens-30's announcement), which is the one place it was ever going to hear the range line.
  - ~~The lens title's accessible name is `<Horizon> lens, <period>. Change lens or period.`~~ **RETIRED — there is no title button to name.**
...
- **R-lens-19 (group rendering: collapse, suppression, and what is not drawn)** ⚠ **DELETED by A10 with R-lens-3.** There are no groups to render, collapse or suppress, so none of the clauses below has a subject. **The carried band's collapse is unaffected** (R-lens-12): it is not a Life-goal group but a different *kind* of content in the same lens, it keeps its header and its `Weekly|__carried|<week>` collapse key, and `CollapsibleHeader` survives for it alone. ⚠ **The suppression clause's generalisation to the goal picker (R-nav-31) is unaffected too** — one non-empty group needs no header *there*, where grouping is still how a picker reads. ⚠ **generalised by R-nav-31 (A6) — the suppression rule is the goal picker's too: one non-empty group needs no header, on either surface** — The group header is one `S.sectionLabel` row: `▾ <LIFE GOAL TITLE> · 3 OPEN` (R-lens-4's count).
...
- **R-lens-20 (an item with no Life ancestor)** ⚠ **REWRITTEN by A10 — there is no `UNSORTED` group and no group note.** An item whose ancestor chain does not reach a Life goal renders, **on its own card**, the muted line **`Not under a Life goal yet`** — the same 12.5px `T.mut` slot R-lens-23's line occupies, and a `<button>` opening the existing Move sheet in `only: 'life'` mode, **which finally gives that mode the caller `25-goal-picker` records it has never had.** Its accessible name is `Not under a Life goal yet. Put it under one.` It **sorts last** (R-lens-5), which preserves `UNSORTED`-last without a header. This is the first time the state has had a keyboard-reachable action at all. `UNSORTED` survives as a word in the goal picker and in Learnings (R-learning-2/3) and on the goal page's trail sheet, which is a different surface asking a different question. ~~A goal whose ancestor chain does not reach a Life goal — a dangling `parentId`, a cycle (R-lens-3's cycle-safe walk), or a migration that could not attach it — groups under **`UNSORTED`**, pinned last, with the line `These aren't under a Life goal yet.` *Renames R-lens-3's `Unattached`;* `Unsorted` is live product vocabulary (R-learning-2/3) and survives Ideas' deletion (R-rm-1).
  - ~~The group carries **no count** and is **never collapsed by default**.~~
  - ~~Each item gains one extra action, `Put under a Life goal…`, opening the existing Move sheet with the Life goals pre-listed.~~ **That action is now the line itself.**
...
- **R-lens-22 (the Zoom sheet's counts)** ⚠ **DELETED by A10.** There is no sheet and there are no counts: five ambient numbers in a permanent strip is a report (R-nav-26), it is width the strip does not have, and the destination itself answers the question better than a number can now that reaching it costs one tap. ⚠ **`GET /goals/zoom` and `GoalService.zoom` have no caller — flagged for removal, deliberately not removed in A10's pass.** ~~modified by A4 — each row also carries its period's span (R-lens-28)~~ — Each Zoom-sheet row shows the number of **goals** at that horizon in the period that row would land on; the Life row reads `everything`. A zero count is omitted, not rendered (the app already omits zero counts).
...
- **R-lens-23 (the line on an item)** ⚠ **REWRITTEN by A10 — it names the LIFE GOAL, at every horizon, with no suppression.** Every item in every lens except Life renders **one muted line naming the Life goal its parent chain reaches** — `under Be genuinely fit at 50` — as a `<button>` opening that goal's page, with the accessible name `under <title>. Open goal.` **No suppression at any horizon.** Full ancestry lives on the goal's own page with its crumb trail (R-goal-41, `22-ux-fixes` §4), never in a lens.
  - ⚠ **Both halves of the old rule broke with R-lens-3.** The suppression had no referent once there were no groups, and a flat Yearly list would have carried **no ancestry at all**, because a Yearly goal's parent is always a Life goal and was therefore always suppressed. The Yearly lens **gains** the line it did not have.
  - **Why the Life goal and not the immediate parent.** It is exactly the string the deleted group header carried, so nothing left the screen — it moved from above a run of cards to inside each of them. It is **one rule at four horizons**, where the immediate parent is a Yearly goal on the Quarterly lens, a Quarterly goal on the Monthly lens and a Monthly goal on the Weekly lens — four different kinds of fact wearing one word. And Life-line titles are short, few and memorised, where intermediate titles are long, varied and read as noise when repeated down a list.
  - **The cost, stated:** on the Weekly lens `under <the month this week serves>` is lost. It is one tap away on the weekly goal's page. This is the one thing A10 takes and does not give back, and the fix if it is missed is a *second* line on that lens, never a per-lens rule.
  - An item with no Life ancestor takes R-lens-20's line in the same slot.
  - ~~It renders on Quarterly, Monthly and Weekly items. A Yearly item's parent is always its group's Life goal, so it never renders there; a Life item has no parent.~~
...
- **R-lens-25 (one gesture, and its keyboard equal)** — A **horizontal swipe** on the lens body steps the period, mirroring the chevrons' direction (left-to-right = earlier).
  ...
  - ⚠ **A10** — the **lens tab strip** is marked `data-h-scroll` **and** `data-no-swipe` (the first is the marker the body's handler checks today, the second is the marker `22-ux-fixes` §6.3 generalises it to), so the period swipe can never fire from inside it under either code path. **Horizontal inside the strip is altitude; horizontal on the body is time** — one control per dimension still, separated by region rather than by axis, and the two attributes make the separation a mechanism rather than a convention. When the unbuilt vertical zoom gesture lands, its always-present non-gesture equal is **the tab strip**, not the Zoom sheet.
...
- **R-lens-31 (the month band — a week shows its month's tasks, and never styles them)** — The Weekly lens for week `W` renders a **third section**, below this week's plan (R-lens-12 case 1) and below the carried band (case 2): the **month band**, headed **`This month · <the month's label>`** — the words *and* the month's own name, never the words alone — and holding the month tasks of **the month `W` belongs to**, grouped by their Monthly goal. ⚠ **AMENDED BY A8’S WEB HALF (`33-measurables-ux` §7.1): the heading names its month.** On 2 Sep 2026 the band reads `THIS MONTH · AUG 2026`, because a heading that will not name the month it is showing makes the Monday rule unreportable at exactly the seam R-lens-29 exists to explain — an owner who sees `THIS MONTH` above August's work has no way to tell a bug from the rule.
  ...
  - ⚠ **AMENDED BY A8’S WEB HALF (`33-measurables-ux` §7.1) — the create affordance, in full. Replaces the retired bullet ~~"`+ Task` renders at the band's foot … Each of the band's Monthly-goal sub-headings also offers `Park in a week` on its rows"~~:**
    - **`+ Task` renders on each band card's foot, never at the band's foot**, and creates a month task on **that card's goal** in **the band's own month** (R-task-57 a). The goal is the card you tapped, so nothing is chosen, nothing is inferred, and no goal picker is needed — a band-foot create would have to ask which Monthly goal, and the only honest way to ask is a fifth R-nav-31 mode.
    - **It renders if and only if the band's month is not past** (R-goal-36, R-task-41, R-task-57) — the same rule that already withdraws every create affordance from a past week and from the carried band. On 2 Sep 2026 the band is August's, so **no card in it renders `+ Task` at all**, and `PERIOD_IN_PAST` is **unreachable from this surface rather than handled on it**.
    - **In its place, the band's foot names the destination and offers one tap to it**, in R-lens-29's shape: `Aug 2026 has ended. New work for the month goes in Sep 2026.` and `Go to Sep 2026`, which navigates to the Monthly lens at the current month. It carries no colour: a month that has ended is a fact about the calendar, not a problem with the plan.
    - **The band never substitutes a month.** It passes its own `monthPeriodKey` and never `currentPeriodKey('Monthly', today)`, never a clamp and never a fallback. Because the control renders only when that month is not past, the two are equal wherever it renders — so there is no branch in which the band creates into a month other than the one it is showing. **A build that adds a clamp here has reintroduced A9's leak in a new place.**
    - **Park is not offered on a band row.** It is on the task page (R-task-56), one tap away, where it can state what it will do. A third control on every row in every list, for an operation done a handful of times a month, is the density R-nav-27 refuses.
  - **The band carries no `Pull from backlog`** (the owner, taking `33-measurables-ux` §8.1's recommendation). A pull is a planning decision about a goal's own deferred work, and the goal's lens and page are where that backlog lives; the band is a week's *view* of a month, not a planning surface for one.
  - **Its contents are ordinary cards with ordinary rows at ordinary size, weight and colour.** Nothing is tinted, dimmed, indented, shrunk or greyed. This product states a difference in **words** and not in a colour (`nothing this week` is the same grey as `3 weekly goals`; no goal is muted anywhere, R-goal-38), and demoting the month's work into a visual footnote is the opposite of what the owner asked for. **Position, the heading that names the month, and one sentence — `Due by the end of Aug 2026, not by the end of this week.` — carry the whole distinction.**
  ...
  - ⚠ **AMENDED BY A8’S WEB-HALF FIX PASS — the band's GOALS are on the Weekly payload (`LensResponse.monthGoals`), and a CARRIED month task is why.** The band draws one card per Monthly goal, and no other field on a Weekly payload can name them: `items` and `carried` are Weekly goals, and `parents` holds the parents of *those* — which incidentally includes a band goal whenever that goal also has a weekly plan, and never otherwise. Reading the **Monthly lens for the band's own month** instead was wrong, not merely slow: a month task carried out of June and open in August still hangs off the **June** Monthly goal (R-task-53), which is not on August's page, so the row was dropped silently and the **whole band vanished when every task in it was carried** — the section absent on exactly the screen carrying exists for. `monthGoals` is one entry per distinct `goalId` in `monthTasks`, in that array's own first-appearance order, resolved out of the interior tree the lens already holds (R-lens-27), so it costs no read; it is empty on every other lens, Monthly included. **The client makes no second lens request, and a fallback one would be the defect re-added.**
    - *Amends R-lens-19's suppression clause, which A10 left standing for the payload even after deleting group headers:* `groups` covers **what the screen shows**, not what `items` holds. A Life goal that reaches this week only through the month band is on this week's screen, so its line is on this week's payload — otherwise the band's card renders with no `under …` line and the client has no way to get one but the read this change deletes. Its `openTasks` stays **0**: S-lens-31-3 is untouched, and a month task is still counted in no group header at any lens.
...
- **R-lens-32 (the Monthly lens shows tasks)** — A Monthly goal's card, in the Monthly lens and on its detail page, renders **its month tasks** — those visible in the selected month (R-task-53) — nested inside the card under a hairline, exactly as a Weekly goal's card nests its week tasks (R-lens-12 case 1). *This is the surface the owner asked for: "in my monthly view i can see the tasks that is part of this month."*
  ...
  - ⚠ **AMENDED BY A8’S WEB HALF (`33-measurables-ux` §7.4)** — the nested list's empty state is **`Nothing on this month yet.`**, and not `WeeklyCard`'s `Nothing on this yet.` A Monthly **goal's page** renders a task list and a backlog block one above the other (R-goal-41 as amended), and two adjacent lists both reading `Nothing on this yet.` is the one place R-backlog-30's distinction is lost in a word.
...
#### Amendment 10 — the lens tab strip, and one create action

- **R-lens-33 (the lens tab strip)** `[new]` — Five tabs, in **horizon order**, `Life` first, in a **full-bleed horizontal scroller** directly below the top-right cluster. `role="tablist"` with `aria-label="Lens"` and `aria-orientation="horizontal"`; each tab a real `<button type="button">` with `role="tab"`, `aria-selected` and `aria-controls="lens-panel"`; the lens body carries `role="tabpanel"`, `id="lens-panel"`, `aria-labelledby="<selected tab's id>"` and `tabIndex={-1}`. The routes are unchanged (R-nav-24): a tablist whose panel is swapped by a router is legitimate, and back and forward still move between lenses, which the Zoom sheet also did.
  - ⚠ **No label may be shortened, abbreviated, truncated, ellipsised, wrapped or scaled down.** The strip is as wide as its content (~390px at Manrope 13/700 with `0 14px` padding and a 16px lead and trail) and **the window scrolls over it**: at 360px it scrolls 30px and clips `Weekly`'s tail at the screen edge; at 390px and above it does not scroll at all. `Quarterly` is never `Qtr`, `Quart…` or 11px type. **That is the whole reason this is a scroller and not a segmented control: a segmented control's only failure mode is truncation, and this layout has no failure mode.**
  - **Active = colour + a 2px rule + `aria-selected`, and the weight does NOT change.** `T.mut` → `T.ink`, plus `boxShadow: inset 0 -2px 0 T.accent` flush to the strip's bottom edge, sharing the strip's own `borderBottom: 1px solid T.line` baseline so it reads as a thickening of the hairline rather than a second line. Weight stays **700** in both states: 700 → 800 changes the glyph advances, which changes the tab's width, which reflows every tab to its right on every selection, and a navigation control whose siblings shift when you use it cannot be aimed at twice. **No new colour token** — inactive `T.mut` on `T.paper` is 4.61:1 (AA at 13px) and active `T.ink` is ≈15:1.
  - **Keyboard** (R-lens-13's surviving accessibility clause, met literally): `Tab` is **one stop** for the whole strip (roving `tabindex`); `←`/`→` move focus and the roving index and **do not activate**, stopping at both ends and **never wrapping**, with `stopPropagation` so the body's `←`/`→` period shortcut cannot also fire; `Home`/`End` reach the ends; `Enter`/`Space` activate. **Manual activation, not automatic** — arrowing `Life` → `Weekly` under automatic activation would fire three route changes, three lens reads and three history entries to reach one destination.
  - **It is mounted ONCE, in the shell above the five lens routes** (a layout route), so **focus and `scrollLeft` survive a lens change**. Inside a per-lens component, activating a tab would unmount the element holding focus and drop a keyboard user to `<body>` — twice per tap, because opening a lens with no period segment renders the index route and then canonicalises.
  - **Scrolling**: `overflow-x: auto`, `scrollPaddingInline: 24`, `overscrollBehaviorX: contain`, no visible scrollbar, and `scrollIntoView({ inline: 'nearest', block: 'nearest', behavior: 'instant' })` on selection. `nearest` does real work — a tab already fully visible **does not move the strip at all**. ⚠ **`behavior: 'instant'`, always, on every platform and every preference: there is no animation to reduce, so there is no `prefers-reduced-motion` branch and the product's "no animation anywhere" line is not crossed.** **No `scroll-snap`** (a settle animation that fights a deliberate drag), **no wrap** at either end (an ordered scale has a first and a last; a carousel would say Weekly and Life are neighbours), **no arrows and no gradient fades** (the clipped glyph at the screen edge is the affordance, and it costs nothing).
  - **Full bleed** — `marginInline: -16` on the strip, `paddingInline: 16` on the track — for two reasons, both wanted: a clipped tab is cut by the **screen** rather than by a box, which reads as *there is more over there*; and the hairline runs the full width, so it is the boundary between *which lens* and *the lens*. Over 640px the bleed stops at `S.page`'s column edge. No media query, no breakpoint.
  - ⚠ **It is STICKY, together with the period row, as one block** (R-nav-27), on the owner's override of `29-ux-navigation` §2.7's recommendation. **Both rows or neither** — never the tabs alone.
  - **It carries no count.** Five ambient numbers, permanently, in a strip that has no width for them, on a product whose rule is that a lens is not a report (R-nav-26). The destination answers the question in one free tap.
- **R-nav-32 (one create action)** `[new]` — A lens has **exactly one** create affordance: **`+ Goal`** in the top-right cluster (R-nav-25), absent on a past period (R-goal-36). `+ <Horizon> goal` is deleted from every group foot, from every card and from the cluster's five-way label; the empty-state CTA is **kept** and relabelled `+ Goal`, because one button on an otherwise empty screen is not clutter, and `+ Sub-goal` on a goal's own page (R-goal-48) is untouched, because it asks a different question — *what hangs off this one* — where the answer is already known. *The owner's words: "we dont need `+ Monthly goal` everywhere as it looks too clutered. instead we can use what we already have top right to add monthly goals where I'll select the lense (defaults to the lense based on my current page) and the goal."*
  - Its sheet is **`New goal`** at every horizon, carrying title, why, a five-chip **HORIZON** selector **defaulting to the current lens**, a **read-only** period chip, and **UNDER** — the one goal picker in `parent` mode (R-nav-31), unchanged.
  - **Changing the horizon does four things, in this order:** (1) the period chip **re-clamps** through `zoomTo(H′, anchor, today)` — **R-lens-9's clamp, the same function the tab strip calls**, so navigation and creation can never disagree about which period a horizon means; (2) the parent picker **re-scopes** to `{ kind: 'parent', horizon: H′, periodKey: <the new period> }`; (3) a chosen parent is kept **iff it is still legal** at `H′` (strictly longer horizon, R-goal-5 / R-goal-32) and is otherwise **cleared with a stated reason** — `Cleared — a Quarterly goal can't sit under a Monthly one.`, `aria-live="polite"`, **never silently**; (4) `Save goal` re-evaluates. Silently re-parenting to the nearest legal ancestor was refused: magic that changes a field you set, for a saving of one tap.
  - **`Life` is the one branch:** no period chip, no `UNDER`, and neither `parentId` nor `periodKey` on the request (R-goal-3).
  - **The period is never typed and never picked**, at any horizon. **`PERIOD_IN_PAST` is unreachable from this sheet by construction**: the anchor is `today` when the period on screen contains today and the period's first day otherwise, so mapping it to any other horizon lands on a period containing that same date — and the create button is already absent on a past period.
  - **If the created goal's horizon is not the lens on screen, the app moves to that lens at that period.** This is R-nav-19's principle, which the old design made unreachable by making the horizon unchangeable: nothing may be created into a period and then vanish from the screen that created it.
  - **On edit the selector does not render at all**: a goal's horizon is immutable — re-parenting is Move, re-scheduling is Re-plan. The heading stays `Edit goal`.
  - **A chip is not a tab, and the difference is deliberate**: *a tab is where you are; a chip is what you chose.* Rendering the sheet's horizon selector as a second tab strip would say the sheet navigates, which it does not. It is `role="radiogroup"` with five `role="radio"` chips, roving `tabindex`, `←`/`→`/`Home`/`End` — the app's existing `S.chipBtn` idiom, literally the control the sheet already renders for `PULSES`.
...
- **R-task-55 (completion is bounded in the task's own scope)** — *Generalises R-task-44.* A completion names a period `P` of the task's `scope` and must satisfy `originPeriodKey ≤ P ≤ currentPeriod`; a later `P` is refused with `WEEK_OUT_OF_RANGE` (the code is unchanged and is now scope-agnostic). `donePeriodKey = P`. A task under a future Monthly or Weekly goal cannot be completed until that period arrives, and its row renders no checkbox (`completable` is unchanged in meaning).
  ...
  - ⚠ **AMENDED BY A8’S WEB-HALF FIX PASS — `Move to Backlog` names a period at the task's own scope too, and D-12's `fromPeriodKey` is that period.** The bound differs (`allowFuture`, R-task-36) and the *scope* rule does not: a Monday against a month task is `WEEK_OUT_OF_RANGE`, so the exit sheet must send a **month key** for a month task. The client had one spelling of "the period to write a task into" and the exit sheet did not use it — it held a week offset and posted a Monday, so **every `Move to Backlog` on a month task was refused**, on a sheet that was already reading `task.scope` to name the goal the item would land on.
  - ⚠ **The month a month task is "in now" is the CURRENT month, bounded below by its origin.** A carried task's origin is behind, so the origin would be a past month; a task written for a month that has not arrived is legal and ordinary (R-goal-36, R-task-57), so the current month would be a period the task did not exist in. `max(originPeriodKey, currentMonth)` is the one answer both bounds admit, it is a comparison of two `YYYY-MM` keys and not a date computation (R-goal-33 chose the format for exactly that), and it has **one spelling in the client** — four callers now: the task page, a task row, the exit sheet and the Park sheet.
- **R-task-56 (Park in a week — the one operation that rewrites a task's scope)** — A **month task** may be parked into a specific week; a **week task** may be moved back to its month. Both are the same command, `retarget`, and both are explicit, logged writes.
  - ⚠ **APPENDED BY A8’S WEB HALF (`33-measurables-ux` §7.2) — where both directions are raised from, and what un-park is called.** **Both are raised from the task page**, from a `WHERE THIS GOES` block that sits above the three exits, separated from them by its own eyebrow and a gap and by nothing else — no border, no card, no colour, because Park is not an exit (R-task-13 is unchanged at exactly three) and a coloured well would say it was. **Park in a week** opens the retarget sheet, which renders the same `When this lands` control the create sheet does (R-task-49 as amended by A11), with **no month option**. **Un-park is one tap with no sheet and no confirm**, and its visible label **names its destination** — `Move to Sep 2026`, not `Move to the month` — because there is no sheet on that path to state where the work lands. It is **withdrawn, not disabled**, when the Weekly goal has no Monthly ancestor (`HORIZON_CONFLICT`) and when the task is not open (R-task-17).
  ...
  - ⚠ **AMENDED BY A8’S WEB-HALF FIX PASS — the weeks Park OFFERS are the weeks of the month the task is IN, not the month it came FROM.** The offer was the weeks of `originPeriodKey`, and a **carried** month task's origin month is behind: every week of it is filtered out as past (R-goal-36), the list came back empty, nothing could be chosen, and `Park it` was permanently disabled — a sheet that opens and can never be finished, on the exact task shape month tasks exist for (R-task-53). The control is **not** withdrawn instead: parking a long-carried task into a week of the month it has reached is the most useful thing this sheet does, and withdrawing it would leave that task no way out of the month but an exit. `park` bounds the target week by `PERIOD_IN_PAST` and by **nothing else** — it does not require the week to be inside the task's origin month — so the offered set is exactly the set the server accepts and **no option in it can be refused**. The sheet's destination line names that month too (`Lands in the week of 14 Sep · Sep 2026.`); naming the origin month would describe a month the write does not touch.
  - ⚠ **AMENDED BY A8’S WEB-HALF FIX PASS — un-park is WITHDRAWN when the monthly ancestor's month is past, and that is a third withdrawal beside the two above.** Un-park's destination is **fully determined by the tree**: the server walks to the nearest Monthly ancestor and then *checks* the key the client sent against that goal's own month, refusing any other with `VALIDATION_FAILED`. So there is no second month this control could name and **"retarget to the current month instead" is not an option that exists** — which leaves exactly two honest behaviours once that month has ended: send a write the server refuses with `PERIOD_IN_PAST`, or take the control away. It is taken away, which is R-lens-31's own answer to the identical shape: **the error is unreachable rather than handled**, so it is not caught, not mapped and not copy-written for. D-5 applies unchanged — no disabled button and no sentence apologising for an option that was never real — and the `In the week of 31 Aug.` line still renders, because where the task lives is a fact that stays true. **It is not the rare case:** at the seam the Weekly lens shows the week of Mon 31 Aug while the calendar month is September, so a task in *this* week under an August Monthly goal meets it.
...
- **R-task-59 (a month task is not a second backlog — the pressure valve)** — The three exits are unchanged and all three work on a month task.
  - ⚠ **APPENDED BY A8’S WEB HALF (`33-measurables-ux` §7.7)** — the exit is the **existing** `Move to Backlog` sheet with **not one string changed**. The only change is which goal it names: for a **month task** the landing goal is the task's **own** goal, because R-backlog-29's walk terminates immediately — so the client's `[...ancestors].reverse().find(a => a.horizon !== 'Weekly' && a.horizon !== 'Life')` must be preceded by *"if the task's scope is `Monthly`, the landing goal is `task.goalId`"*, or the sheet names a **grandparent** while the write lands on the parent. In particular **Move to Backlog** on a month task lands the item on **that same Monthly goal** — the nearest goal that can hold backlog is the goal it is already on (R-backlog-29's walk terminates immediately) — and `fromWeek` is `null`, because the task had no week. It renders `from Sep 2026` rather than `from week of …`.
...
- **R-measure-4 (progress, the optional target, and `target == start`)** — Progress is **one formula for both kinds**: `(current − start) / (target − start)`. It handles both directions without a branch: `start 80, target 75, current 78` is `(−2)/(−5) = 0.4`.
  ...
  - ⚠ **APPENDED BY A8’S WEB HALF (`33-measurables-ux` §7.5) — in the UI these are TWO INDEPENDENT KEYS and must never be collapsed into one.** The `current / target` form is keyed on **`target !== null`**; the bar is keyed on **`progress != null`**. A no-target gauge has **neither** — it reads `24 reps`, with no slash, no percentage, no bar and nothing on screen mentioning the target it does not have. A measure with `target === start` in the data has **the numbers and no bar** — `62 / 62 leads` — because the numbers are what was stored and no division was performed. Keying the numbers off `progress` renders that task as `62 leads`, which is a lie about its data.
...
- **R-measure-9 (what an agent may do with a measure, and what it may not)** — The MCP surface reads and updates measures like any other field: attach, edit and remove a measure, record and delete a reading, read the triple and the history. The server-instructions block teaches the model — the two kinds, the implied direction, the optional target, the derived `current`, and that readings follow the task.
  - ⚠ **APPENDED BY A8’S WEB HALF (`33-measurables-ux` §7.6) — three UI-shaped refusals, named so they can be audited:**
    - **No percentage is rendered anywhere**, in any surface, at any size. `21%` discards the unit — the one word that makes the owner's number theirs — invites `100% = done`, is the natural home for a colour that means good or bad, and is a number *the app derived*.
    - **The bar carries no `role="progressbar"`.** That role's `aria-valuenow` is announced as a percentage by most screen readers, which would make the one forbidden number audible while it is invisible.
    - **Reaching the target renders and announces nothing.** No sentence, no colour change, no icon, no live region. The numbers change and the bar is full; the product has no comment. The absence is the feature: a sentence the app writes about your number, at the moment your number gets interesting, is the app having an opinion — and it would be the seed of the next one.
...
- **R-backlog-31 (conversion targets the goal you are standing on, or a week)** — A backlog item on a **Monthly** goal offers two conversions; one on a Yearly or Quarterly goal offers only the second. *Amends R-backlog-26, R-backlog-7 and R-backlog-8, none of whose week-path rules change.*
  ...
  - ⚠ **AMENDED BY A8’S WEB-HALF FIX PASS — a conversion carries a `measure` the command NAMES, on both paths.** `ConvertBacklogItemRequest` gains an optional `measure`, the twin of `CreateTaskRequest.measure`, and the task comes back measurable with its `Measure added` line in the same batch (R-task-58). **Nothing is inferred from the item**: a backlog item has no number to carry, and inventing one would be the app deciding what the owner meant, so absent still means an ordinary checkbox and a conversion with no measure named is byte-identical to what it was before A8 (S-measure-1-1). It is on the wire because **the create sheet is one sheet with two save paths** (Q-E): it renders the same measure fields on both, validates them on both and **blocks its save button on their validity** on both, so without this field the client had to either discard a number the owner was required to get right — silently, which is A9's own defect in a new place — or hide the fields on one of two doors to the same task, for no reason the owner can see. `MEASURE_TARGET_EQUALS_START` applies identically on both paths, from **one** enforcement point (`domain/measures.assertMeasure`), which moved out of `TaskService` for exactly this reason: "the whole enforcement point" was true while one service wrote measures and stopped being true the moment a conversion could carry one.
...
- **R-nav-25 (the top-right cluster)** ⚠ **modified by the reconciliation pass — the lens create button names its horizon; ⚠ its GOAL-DETAIL mapping is superseded by R-nav-29 (A3) — `+ Task` on a Weekly goal and nothing at any other horizon, sub-goals having moved into the section that holds them (R-goal-48)** — Unchanged in form (R-nav-11): the theme toggle plus **at most one** primary action per page. ⚠ **AMENDED by A10 — a lens's one primary action is `+ Goal`, the same string at every lens**, absent (not disabled) on a past period (R-goal-36). The button no longer commits to a horizon, it *defaults* to one (R-nav-32), and a label that names a default as if it were a destination is a label that lies; it is also 46px instead of 96px, which is width the cluster row did not have. **The five horizon-named labels are retired**, on the cluster, in every empty-state CTA and at every group foot (R-nav-32). Goal detail and the task page mappings are untouched (R-nav-29). ~~The mapping changes with the screens — Goals → `+ Life goal` · `+ Yearly goal` · `+ Quarterly goal` · `+ Monthly goal` · `+ Weekly goal`, per the current lens, pre-filled with its horizon and selected period, and **absent** (not disabled) on a past period (R-goal-36);~~ Goal detail → `+ Weekly goal` on a Monthly goal, `+ Task` on a Weekly goal, `+ Add` (backlog) on Yearly/Quarterly, none on Life; Backlog → `+ Add`; **Task page → the theme toggle only**, which today's goal detail omits and both must carry; Learnings → none.
...
- **R-nav-27 (three rows of chrome, and nothing else unconditional)** ⚠ **REWRITTEN by A10 — the budget goes from two rows to three, knowingly, and the third is paid for** — Above the first item of any lens there are **at most three unconditional rows**: the top-right cluster (R-nav-25), the **lens tab strip** (R-lens-33) and the period row (R-lens-7). Everything else is conditional and mutually exclusive: the off-now row only off-now (R-lens-21) or the week-elsewhere row only when current (R-lens-29). **There are no group headers at any horizon, and no per-group create.** **A fourth unconditional row is refused, not deferred.**
  - **The ledger, at 360px, on an account with more than one Life line:** cluster 1 → 1, **lens tab strip — → +1**, period row 1 → 1, group header 1 → **−1**. **Total 3 → 3.** Also removed, below the first card but inside the list: one `+ <Horizon> goal` link row **per group**, repeated down the page — the owner's literal complaint; the `why` line on every card at four of five horizons (R-lens-23's line takes its place, so a Monthly card is one line **shorter**); and a whole navigation surface, the Zoom sheet, with the tab stop and control that opened it.
  - **The honest exception:** an account with exactly **one** Life line rendered no group header (R-lens-19), so it goes from **two** rows to **three**. That row is spent knowingly. It buys the removal of a two-tap modal from the single most frequent navigation act in the product, and it is the row the owner asked for by name.
  - ⚠ **A10 — the tab strip and the period row are STICKY, as one block.** `29-ux-navigation` §2.7 recommended not sticky and warned that if it were overturned the correct form is *both rows together, or neither* — a strip that sticks without its period row lets you change lens but not period from the same place, which splits one header into two behaviours depending on scroll position. **The owner overturned it**, so the two rows sit in one `position: sticky` container and there is no scroll position in which one is pinned and the other is not. The permanent cost is stated because it is permanent: **44px of tabs + a 1px hairline + a 46–51px period row ≈ 91–96px** held for the whole scroll on a ~700px phone viewport. Nothing else in the product is sticky except the bottom tab bar.
...
#### Nav and Lens — Amendment 10

- **S-lens-33-1** (R-lens-33, happy) — *Given* any lens. *Then* a `role="tablist"` named `Lens` renders five tabs in horizon order, the current lens is the one with `aria-selected="true"`, and **one tap on any other tab changes lens** — no sheet is opened on the way and none is left behind.
- **S-lens-33-2** (R-lens-33, happy) — *Given* a 360px viewport. *Then* each of `Life`, `Yearly`, `Quarterly`, `Monthly`, `Weekly` renders **in full**: its text content is the whole word, it is `white-space: nowrap` and `flex-shrink: 0`, and it carries no `text-overflow`, no `max-width` and no reduced type size. Nothing is shortened, truncated or scaled; the track scrolls instead.
- **S-lens-33-3** (R-lens-33, happy) — *When* the strip has focus. *Then* it is **one** tab stop; `←`/`→` move focus **without** changing the lens (`aria-selected` stays where it was); `Home`/`End` reach the ends and neither wraps past them; `Enter` changes the lens; and the change is announced by the tab's own selected state plus the live region's `<period>. N goals.`
- **S-lens-33-4** (R-lens-33 / R-nav-27, happy) — *Given* a lens whose list is long enough to scroll. *Then* the tab strip and the period row are inside **one** `position: sticky` container, so both stay pinned together, the list scrolls beneath them, and the strip is still an `overflow-x: auto` scroller inside it.
- **S-lens-17-1** (R-lens-17, unhappy) — *Then* the period title is not a control: it opens nothing, carries no `▾`, and `U+25BE` appears nowhere in the product. `ZoomSheet.tsx` does not exist, no dialog named `Change lens` can be opened from any lens, and `Jump to now` renders nowhere.
- **S-lens-3-4** (R-lens-3 / R-lens-19, unhappy) — *Then* **no Life-goal group header renders at any horizon** — no `<TITLE> · N OPEN` row, no collapse toggle over a run of cards, no `UNSORTED` header and no group note — on any account, at any group count.
- **S-lens-5-2** (R-lens-5, happy) — *Given* items from two Life lines at one horizon and period, created in an order that interleaves the lines, plus one item with no Life ancestor. *Then* `LensResponse.items` arrives with each line's items **contiguous**, the lines in their Life goals' `createdAt` order, the items in their own `createdAt` order within a line, and the root-less item **last** — which is the previously grouped screen's reading order with its headers removed.
- **S-lens-4-2** (R-lens-4, happy) — *Given* a Life goal with open tasks this week. *Then* `N open` renders on **its card in the Life lens** and on no card and no header at any other lens. A zero is not rendered anywhere.
- **S-lens-23-3** (R-lens-23, happy) — *Given* any item at any horizon except Life. *Then* it renders exactly one muted line naming **the Life goal its chain reaches**, as a button whose accessible name is `under <title>. Open goal.` — including on the **Yearly** lens, which previously rendered none, and in the carried band.
- **S-lens-20-2** (R-lens-20, happy) — *Given* an item whose chain reaches no Life goal. *Then* its card carries `Not under a Life goal yet` as a button opening the Move sheet in `only: 'life'` mode, and the item sorts **last** in the flat list.
- **S-nav-32-1** (R-nav-32, happy) — *Given* the Monthly lens. *When* `+ Goal` is used and the horizon is changed to `Quarterly`. *Then* the period chip re-clamps to the quarter containing the anchor, the reason line reads `Closest to <month>, the month on screen.`, a parent that is no longer legal is cleared with `Cleared — a Quarterly goal can't sit under a Monthly one.` in an `aria-live` region, and the picker re-scopes to `Life` and `Yearly` only.
- **S-nav-32-2** (R-nav-32, happy) — *When* a goal is saved at a horizon other than the lens on screen. *Then* the app navigates to that lens at that period, so nothing is created into a period and then invisible.
- **S-nav-32-3** (R-nav-32, unhappy) — *Then* **no `+ <horizon> goal` control renders inside any lens list**, at any horizon, and the one create affordance on a lens is `+ Goal` in the cluster, outside the tab panel.
- **S-goal-46-3** (R-goal-46, happy) — *Given* two Life lines that each held Weekly goals last week. *When* `Repeat last week` is used at the foot of the Weekly list with **no** `lifeGoalId`. *Then* both lines' goals are copied into the selected week as ordinary new goals with `pulse` reset and no tasks; *and when* a `lifeGoalId` **is** supplied, only that line's are — unchanged.
...
#### The web half, and the seven rules it amended (`docs/work/34-measurables-web/build.md`)

The API half shipped first (`docs/work/31-measurables-api/build.md`); the web half was designed by
`docs/work/33-measurables-ux/UX-PLAN.md`, built against that contract, and **amended seven rules in §2**,
each marked `⚠ AMENDED BY A8’S WEB HALF` in place:

| Rule | What changed | Why |
|---|---|---|
| `R-lens-31` | the heading **names its month**; `+ Task` moves to the **card's** foot and renders **iff the month is not past**, with a past-month foot in its place; no `Pull from backlog`; no Park on a row; the band's contents are ordinary cards at full weight | a heading that will not name the month it shows makes the Monday rule unreportable at the seam; and the affordance that would need a clamp is the affordance that must not render |
| `R-lens-32` | the nested list's empty state is **`Nothing on this month yet.`** | a Monthly goal's page renders a task list and a backlog block one above the other, and `Nothing on this yet.` twice loses R-backlog-30's distinction in a word |
| `R-goal-47` | a new **`No weeks yet`** case, and the line's position settled **above** the task list | `Nothing planned yet` above four visible month tasks is a sentence the screen contradicts |
| `R-task-56` | both directions raised from the task page's `WHERE THIS GOES` block; un-park is **one tap** whose visible label **names its destination** | there is nothing to choose on the way back, and a one-tap write that does not name where it lands is A9's own defect |
| `R-task-59` | the demote's landing goal is the month task's **own** goal | R-backlog-29's walk terminates immediately; the ancestor walk would name a grandparent while the write landed on the parent |
| `R-measure-4` | the slash and the bar are **two independent keys** | collapsing them renders `62 / 62 leads` as `62 leads`, which is a lie about the data |
| `R-measure-8` | no percentage anywhere; no `role="progressbar"`; **reaching the target renders and announces nothing** | each is a way the forbidden number, or the app's first opinion, gets back in |

**`docs/BUSINESS-RULES.md` was unchanged by the web half as originally built.** A11 §8.6's sentence and
A8's own month-band and Measure paragraphs already said what that build shipped — *"Below both of those
sits **this month**…"*, *"**Nothing in that month band wears a late label**…"* and the whole Measure
section. ⚠ **The fix pass below changes two of its bullets**, and
`apps/api/src/api/mcp/business-rules.ts` is regenerated in the same commit;
`apps/api/tests/mcp/verbatim.test.ts` is the alarm that catches a commit where it is not.

#### The web half's fix pass — five defects, and what each cost the contract

Found by review against the branch, before anything merged. Two of the five needed the wire to change,
and both changes are **additions to a response and a request** — no field changed meaning, no field left.

| # | The defect, at its root | The fix | Wire? |
|---|---|---|---|
| 1 | **A carried month task rendered nowhere, and could take the whole band with it.** The band read the *Monthly lens for its own month* to find its goals, and a carried task's goal lives in an **earlier** month, so the row was dropped silently and the band returned `null` when every task in it was carried — the section absent on exactly the screen carrying exists for (R-task-53). | `LensResponse.monthGoals`, built from ids `GoalService.lens` already holds, with `groups` widened to cover them. The second lens read is **deleted, not kept as a fallback**. | **yes** — `LensResponse` gains `monthGoals: GoalView[]` (R-lens-31) |
| 2 | **`Move to Backlog` on a month task was refused every time.** The exit sheet held a week offset and posted a **Monday**; a Monday against a month task is `WEEK_OUT_OF_RANGE` (R-task-52). The sheet was already reading `task.scope` to name the landing goal, so the fact was on screen while the write was wrong. | the sheet sends a period at the task's own scope, through the client's **one** spelling of that rule (R-task-55, amended). | no |
| 3 | **`Move to <month>` was an impossible write whenever the ancestor's month was past** — and a green test pinned it, because the MSW handler did not replicate `PERIOD_IN_PAST`. | the control is **withdrawn**, R-lens-31's own answer to the identical shape. Retargeting to the current month is not an option that exists: the server derives the destination from the tree and refuses any other key (R-task-56, amended). | no |
| 4 | **`Park in a week` dead-ended for a carried task**: the offer was the weeks of the **origin** month, all past, so the sheet opened and could never be finished. | the offer is the weeks of the month the task is **in now** — exactly the set `park` accepts (R-task-56, amended). | no |
| 5 | **A measure attached during a backlog conversion was silently discarded.** The sheet rendered the fields, validated them and **gated its save on their validity** — then threw them away, because `ConvertBacklogItemRequest` had no `measure`. | the request gains one, explicit-only; `assertMeasure` moves to `domain/measures` so both minting paths share one enforcement point (R-backlog-31, amended). | **yes** — `ConvertBacklogItemRequest` gains `measure?` |

**Six test-quality gaps were closed in the same pass**, all in this build's own tests and all of the same
kind — an assertion that could not fail for the reason it claimed: a suppression prop asserted only where
the whole row was already gone; a six-word blacklist standing in for *the governing rule of the feature*
(now the announcing region's full `textContent` **equals** the ordinary sentence); an enumerated list of
forbidden SVG children that missed `<title>`, the tooltip `Sparkline.tsx`'s own docblock forbids by name
(now `svg.querySelectorAll('*')` has length **1**); a whole-string `queryByText('Recorded')` that no real
toast could ever match (now `queryByRole('status')`); three tests that passed identically against `main`;
and a carry-suppression test that matched strings at one age rather than the rule at both.
...
### Amendment 10 — lens tabs, flat lists, one create action, sticky nav

Four navigation changes the owner asked for after using the redesigned app daily. **Three of them overturn
decisions taken in `docs/work/14-redesign`, and one overturns an instruction the owner gave themselves.**
Every reversal is named below, with the sentence being reversed quoted. The design is
`docs/work/29-ux-navigation/UX-PLAN.md`, which was written first and followed literally.

**The owner:**

> *"btw i find it difficult navigating. i dont need to click on a dropdown to change the lense as it adds
> friction. instead can we have a tabs in the top where i dont need double clicking to change lense and
> below that i can change the timestamp as normal. and we dont need `+ Monthly goal` everywhere as it looks
> too clutered. instead we can use what we already have top right to add monthly goals where I'll select
> the lense (defaults to the lense based on my current page) and the goal. also lets not categorise based
> on life in any horizon."*

> *"yes but i mean there should be a ux solution there like probably when i start clicking on a tab moving
> towards right the option start scrolling left."*

The four:

1. **The lens is a tab strip** (R-lens-33, R-lens-17 rewritten). Five tabs in horizon order, one tap, in a
   full-bleed horizontal scroller with the selected tab kept in view. **The Zoom sheet is deleted in
   full** — the file, `useZoom`, the `['zoom', anchor]` key, the `▾` marker and the title's button.
2. **One create action** (R-nav-32, R-nav-25 amended). `+ Goal` in the cluster at every lens, whose sheet
   carries a five-chip horizon selector defaulting to the lens, a read-only period chip clamped by
   R-lens-9, and the existing horizon-scoped parent picker.
3. **No grouping by Life goal, at any horizon** (R-lens-3 and R-lens-19 deleted; R-lens-4, R-lens-5,
   R-lens-20 and R-lens-23 rewritten). Every lens is a flat list. **The order does not change**: it is the
   grouped screen's reading order with its headers removed. Orientation moves onto the card as
   `under <Life goal>`; the open-task counts move to the Life lens's cards and are deleted everywhere else.
4. **Card density** (R-lens-23, R-nav-27). **`why` leaves the four working lenses and stays on Life,
   clamped to one line**, paying line for line for the `under` line item 3 adds — so a Monthly card ends up
   one line *shorter* than it was.

**The owner's one override.** `29-ux-navigation` §2.7 recorded stickiness as its open question 1 with
`[recommended] not sticky`, and warned that if it were overturned the correct form is **both rows together
as one block, or neither**. The owner answered: **both rows sticky.** Built as one `position: sticky`
container, so there is no scroll position in which one is pinned and the other is not. The permanent cost
is stated in R-nav-27: **≈ 91–96px**.

**Added:** 2 rules (`R-lens-33`, `R-nav-32`). **Total rules 256 → 258, less 4 retired outright = 254.**
**Scenarios added:** 12 — `S-lens-33-1 … S-lens-33-4`, `S-lens-17-1`, `S-lens-3-4`, `S-lens-5-2`,
`S-lens-4-2`, `S-lens-23-3`, `S-lens-20-2`, `S-nav-32-1 … S-nav-32-3`, `S-goal-46-3` (362 → **374**).
**Retired outright:** 4 — `R-lens-3` (grouping by Life goal), `R-lens-19` (group rendering and collapse),
`R-lens-22` (the Zoom sheet's counts), and R-lens-17's Zoom sheet as a surface.
**Existing rules modified, each marked `⚠` in §2:** 10 — `R-lens-4` (rewritten: the count's one home),
`R-lens-5` (rewritten: the flat total order, now load-bearing on the wire), `R-lens-13` (clarified: its
bottom-tab-bar refusal stands; it does not forbid a lens tab strip), `R-lens-17` (rewritten: the control is
a tab strip and the title is text), `R-lens-20` (rewritten: a line on the card, not a group),
`R-lens-23` (rewritten: it names the Life goal, with no suppression), `R-lens-25` (amended: the strip is
`data-h-scroll` and `data-no-swipe`), `R-nav-25` (amended: `+ Goal`, one string at every lens),
`R-nav-26` (amended: the permitted-numbers list loses one entry and gains none), `R-nav-27` (rewritten:
three unconditional rows, and they are sticky), `R-goal-46` (amended: once at the list foot, `lifeGoalId`
optional).
Running totals across all ten amendments: **131 rules superseded, retired or modified**, **37 retired
outright**.
**No new colour token, type size, dependency, migration, endpoint, error code or MCP tool.** One wire
schema loosens (`RepeatWeekRequest.lifeGoalId` becomes optional), one wire field's **order** becomes
load-bearing (`LensResponse.items`), and one endpoint loses its only caller.

#### The reversals, each with the sentence being reversed

| Reversed | The sentence | Why it falls |
|---|---|---|
| `14-redesign` §1.2 / §1.4 / §10, and R-lens-13's extension | *"A five-way segmented control, pinned under the header. Rejected on the complaint itself… `Life · Yearly · Quarterly · Monthly · Weekly` is 42 characters; at 360px it needs 7px type or truncation."* | Overturned **on evidence**: the owner used the two-tap sheet daily and found it high-friction, which is the only test that matters. Three of its four arguments are *answered* rather than dismissed. **"42 characters"** counted a sentence with ` · ` separators; as tabs there are none — 32 characters, ~358px of tabs in a 390px track. **"7px type or truncation"** — neither: the strip scrolls and nothing is scaled or cut except by the screen edge, deliberately. **"four of five labels are always wrong"** — a tab is a destination, not an assertion; four of five bottom-bar tabs are also "wrong" at any moment. **"it treats an ordered scale as five peers"** is the one that survives, and it is answered by **form**: the tabs are in horizon order, left to right, matching the Zoom sheet's own ladder, `Shift+↑`/`Shift+↓` and `HORIZONS`. An ordered strip is a scale. |
| `14-redesign` §1.3 | *"the Zoom sheet is not one tap too many"* | It is **two** taps (open, choose), in a modal, on the act the owner performs most. R-lens-17 replaces it and keeps its one real promise better: the period row answers *where did I land* in the same frame, **after** a free and reversible tap instead of before a committing one. |
| The owner's own instruction, from `14-redesign` §4 | *"in each lense we donot need a filter on goals instead it will be catogrised by life goals"* → *"also lets not categorise based on life in any horizon"* | The owner reversed themselves after use. The filter pills stay deleted (R-rm-4); what goes is the categorisation that replaced them. R-lens-23's line, R-lens-4's move to the Life lens and R-lens-5's unchanged order are the three replacements. |
| `14-redesign` §6.7 | *"two entry points per lens… the per-group one is the good one and it is why grouping earns its keep"* | With grouping gone the per-group create has no group, and the owner named the repetition as the clutter. The knowledge it supplied — *which line* — is now supplied by A9's nearest-legal-ancestor preselection and by the grouped, searchable picker, which is strictly more capable than a button that guessed. |
| `R-lens-13`'s surviving refusal | *"it is not a tab and must never become one"* | **Not overturned — clarified.** Read in full, that clause is about the **bottom tab bar**: *"five lenses in a five-item tab bar leaves no room for capture or Learnings, and the tab bar is a top-level destination switcher, not a zoom."* That sentence stands verbatim: `Goals · + · Learnings` is unchanged and the lens strip is inside the Goals screen. What is overturned is the extension of the argument to any tab-shaped control anywhere. Its **accessibility** clause is not overturned at all — R-lens-33 is its most literal implementation in the app. |
| `29-ux-navigation` §2.7's own recommendation | *"[recommended] Not sticky."* | **Overturned by the owner.** The plan's condition is honoured: both rows or neither. |

#### Consequences checked and found to hold

- **R-lens-9 / R-lens-18 (the clamp and the anchor)** — untouched, and now **more** exercised: the tab
  strip, `Shift+↑`/`Shift+↓` and R-nav-32's horizon selector all call `zoomTo` with the derived anchor, so
  navigation and creation cannot disagree about which period a horizon means. The keyboard zoom used to
  navigate with **no period at all**, which silently discarded the anchor; that is fixed in passing.
- **R-lens-15 / R-rm-4 (no filters in a lens)** — untouched. A flat list has no filter and no `All` chip;
  removing the grouping does not reopen the question the pills lost.
- **R-lens-16 / R-lens-27 (no read loads the whole goal list)** — untouched. A10 adds no read, no endpoint
  and no query parameter. R-lens-5's flat order is a sort of the page already in hand.
- **R-lens-12 (the carried band)** — untouched, and it **keeps its header and its collapse**. It is not a
  Life-goal group: it is a different *kind* of content in the same lens, and R-lens-12's whole point is
  that the two are never mixed. Deleting its header would merge them. `CollapsibleHeader` survives for it.
- **R-lens-29 (`This week is in Aug 2026 · Go there ›`)** — untouched, and deliberately **not** pinned: it
  is a notice about the period you are on, not a control, and sticking it would be R-nav-27's fourth
  unconditional row in all but name.
- **R-nav-24 (the routes)** — untouched. The five lens routes now sit inside one **layout route**, which is
  a tree shape rather than a URL shape: every address, every back and forward and every pasted link
  behaves exactly as before.
- **R-goal-3 / R-goal-5 / R-goal-32** — the horizon selector's legality is `permittedHorizons` read a
  second way, so no chip can offer what the server would refuse (D-5), and `Life` renders no period and no
  parent.
- **R-nav-31 (the one goal picker)** — **unchanged, and not forked.** R-nav-32 calls it in `parent` mode
  with a different `horizon` and `periodKey`; nothing about the picker changes. Its `only: 'life'` mode
  finally gains the caller `25-goal-picker` records it has never had — R-lens-20's line.
- **R-nav-13 (one toast)**, **R-nav-15 (`Sheet`'s contract)** — untouched. The strip is not an overlay, so
  it adds no focus trap and no `aria-modal`; the create sheet is the same `Sheet` it always was.
- **Contrast (`tests/screens/contrast.test.ts`)** — **no new colour token anywhere.** Inactive tabs are
  `T.mut` (4.61:1 on `paper`), active are `T.ink`, the 2px indicator is `T.accent` and is not the only
  carrier of state, and the hairline is `T.line` and decorative.
- **`docs/BUSINESS-RULES.md`** — the Lens section changes (grouping, the group header's count, the lens
  switcher and the Zoom sheet), and one Weekly-lens sentence changes (`Repeat last week`).
  `apps/api/src/api/mcp/business-rules.ts` is regenerated in the **same commit**, and
  `apps/api/tests/mcp/verbatim.test.ts` is what proves it.

#### Flagged for a later pass, not built here

- **`GET /goals/zoom` and `GoalService.zoom` have no caller.** The client's `useZoom` and its query key are
  deleted; the endpoint, the service method and the `ZoomResponse` schema are **left standing** so this
  pass changes no route surface. They should be removed, not left serving nobody.
- The **vertical zoom gesture** `22-ux-fixes` §6 designed and did not build: when it lands, its
  always-present non-gesture equal is the tab strip, not the Zoom sheet (R-lens-25, amended).
