...
- **R-goal-34 (what "current" means)** — The current period of a horizon is derived from the owner's **account timezone** (R-auth-5), never the client clock: `today = dateInTimezone(now, tz)`, then Yearly = `today`'s year, Quarterly = `today`'s quarter, Monthly = `today`'s month, Weekly = `weekStartOfDate(today)`. Life has no current period. Every past/future judgement in A2 is a `periodKey` comparison against these, computed server-side and echoed on the wire so the client never re-derives one — the disagreement R-auth-5 exists to prevent, now four times over instead of once.
...
- **R-goal-38 (active and dormant, restated per week)** — Both words survive, defined against a **week** `W` and against nothing else:
  ...
  - **Dormancy has exactly one surface, and it is not styling.** No goal is ever muted, greyed, or labelled `DORMANT` anywhere in the product: that muted tree is the clutter this redesign exists to remove. The single consumer is R-lens-6's empty-group line in the Weekly lens. `GoalView.isActive`, `.dormant` and `.subtreeActive` leave the wire (R-rm-2).
  - **A Monthly goal with no Weekly children gets no special affordance** and no different styling from one that has them. It is not broken; it is a month nobody has broken into weeks yet. The only difference anywhere is R-nav-25's primary action, which is `+ Weekly goal` on every Monthly goal alike.
...
- **R-goal-43 (a Weekly goal can go stale, and says so quietly)** — For a Weekly goal, `plannedAgeWeeks = weeksBetween(weekStartOf(updatedAt), periodKey)`. Once its week has **arrived** (`periodKey ≤ currentWeek`), a goal with `plannedAgeWeeks ≥ 2` renders a muted line `planned <N> weeks ago` beneath its title, in the Weekly lens and on its detail page. Muted text only: never a chip, never coloured, never blocking. Age 1 is ordinary planning and carries no label; a week that has not arrived carries none either — it is early, not stale. *Supersedes R-plan-19, which said this of a focus row.*
...
- **R-lens-4 (the group header)** — Each group header shows the Life goal's title, its pulse dot, and its **open-task count** — the surviving home of the counts the filter pills carried (R-nav-7, owner decision 7). The count is the number of **open tasks under that Life goal visible in the anchoring week**: the **selected** week in the Weekly lens, and the **current** week in every other lens, which have no week of their own.
  ...
  - The count is `0`-suppressed only in the sense that it renders `0`; the header is never hidden (R-lens-6).
- **R-lens-5 (order)** — Groups are ordered by their Life goal's `createdAt` asc then `id` asc — the same total, stable order as every sibling list (Q-7) — with `Unattached` last. Within a group, items are ordered by `createdAt` asc then `id` asc. Every item in a group already shares one `periodKey`, so no period tie-break is needed. No list order in a lens depends on storage order, and no lens is manually re-orderable (manual order exists for backlog items and nowhere else, R-backlog-21).
- **R-lens-6 (empty states — two, and they are different things)** —
  - **An empty group** — a Life goal with no items in the selected period — still renders its header and its count, with one muted line: `Nothing at this horizon for <period>.` A Life goal is **never hidden from a lens**: a hidden group is indistinguishable from a deleted goal, and its header carries a count that is true whether or not the group has items. In the Weekly lens this line is the *only* surface dormancy has (R-goal-38).
  - **An empty lens** — no items in any group for the selected period — reads `Nothing at this horizon for <period>.` with the horizon's create CTA, e.g. `[+ New weekly goal]`. A **past** period gets the copy **without** a CTA: `Nothing was set for <period>.`, because nothing may be created into it (R-goal-36).
...
- **R-lens-7 (the period control)** — Every lens except Life carries one period control: a back chevron, the selected period's label (tap to open a picker), a forward chevron. It steps by the lens's own unit — a year, a quarter, a month, a week.
  ...
  - **There is no backward bound either.** Every past period is readable; the back chevron is disabled only at the account's first period. *Retires `WEEK_HISTORY_WEEKS` as a bound.*
  - **D-24's rule survives, restated:** the chevrons and the picker address exactly the same range, and no period is reachable by one control and invisible to the other. With both bounds gone the picker cannot be a fixed chip row — it lists a **window of periods centred on the selected one** and re-centres as it moves, so every period stays reachable from both controls. The window size is presentation, not contract.
  - A period holding at least one item, or at least one task originating in it, is **marked with a dot** in the picker. Without it a goal written three months out is invisible from every screen except that month's (R-nav-18's reason, generalised to five horizons).
  - `Today` / `This week` returns the lens to its current period in one action.
- **R-lens-8 (where a lens opens)** — A lens opens on its **current period** (R-goal-34). The selected lens and period survive within a session and are restorable from the URL (R-lens-14); they are not persisted across sessions, because where you were last Tuesday is not where you want to start.
- **R-lens-9 (zooming between lenses)** — The lens switcher carries an **anchor date**, not a period label; each lens renders the period containing that anchor. Selecting a lens maps it:
  ...
  - **Zoom in** (longer → shorter, e.g. Quarterly → Monthly): if the current period **contains today**, the new period is the one containing **today**. In `Q3 2026` on 31 Aug, zooming to Monthly gives `Sep 2026`, not `Jul 2026`. Otherwise the new period is the **first** sub-period of the current one: `Q1 2027` → `Jan 2027`; `Sep 2026` → the week containing 1 Sep. *This is the answer to "if I am in Q3 and zoom to Monthly, which month?": the one you are living in, and only failing that the first.*
  - Zooming **into Weekly** from a month yields the week containing today when today is in that month, else the week containing the 1st. That week's Monday may fall in the previous month — weeks are Mondays and they straddle (D-1) — and that is correct, not something to round away.
  - **Life has no period**, so zooming to Life discards the period; zooming out of Life uses **today** as the anchor.
...
- **R-lens-13 (the lens switcher)** — A five-way control — `Life · Yearly · Quarterly · Monthly · Weekly` — in the Goals screen header, above the period control. **It is not a tab and must never become one**: five lenses in a five-item tab bar leaves no room for capture or Learnings, and the tab bar is a top-level destination switcher, not a zoom. One tap moves one lens. It is a single tab stop with `←`/`→` between options — the roving-tabindex pattern R-backlog-22 already requires — and the selected lens is announced, not merely coloured.
- **R-lens-14 (every lens, period, goal and task is addressable)** — A router is adopted (owner decision 6). **Routes**, each restorable by URL and by back/forward: the lens (`lens` + `period`), a goal detail page, a **task page** (R-task-45), the Backlog page, Learnings. **Overlays**, not routes, because each is a two-second interaction whose URL nobody wants: the `+` capture drawer, every confirm sheet, the create and edit forms, the period picker. A deep link to a lens+period lands on that lens with that period selected; an unparseable or absent period falls back to the current one rather than erroring.
...
- **R-task-41 (where a task may be created)** — A task is created **under a Weekly goal**, from three sources: (a) `+ Task` on a Weekly goal — in the Weekly lens or on its detail page; (b) a **Backlog pull** (R-backlog-26); (c) the `+` drawer's `Add to this week instead` (R-backlog-27). *Supersedes R-task-2's four sources — the Idea source is deleted with the entity (R-rm-1) — and R-task-4's target selector, which now lists Weekly goals for the target week, labelled `<Life root title> — <weekly goal title>`.*
...
- **R-task-45 (the task page)** — Task detail is a **routed page**, not a drawer (CR-5, owner decision 6). It shows the goal path (Life root › … › Weekly goal, **with that goal's week**), the done date when done, editable `title` / `cond` / `desc`, the links list with add and remove, the three exits when open, and the read-only activity timeline. *Supersedes R-task-22.*
...
- **R-task-48 (one step, not two — the capture rule)** — Creating a task now presupposes a Weekly goal, and the common case is "I need to do this, this week". That must stay **one interaction**, and it is solved in the flow rather than in the model: the task-create sheet, when no Weekly goal exists for the target week under the chosen parent, offers an inline `New weekly goal` field in the **same** sheet, and the save creates both rows **in one transaction**.
  ...
  - This subsumes the `[Add a weekly goal]` action on R-backlog-26's refusal sheet: a conversion with no Weekly goal for the target week offers the same inline field rather than sending the owner away and losing the flow.
...
- **R-nav-24 (the router)** — Screen and overlay stop being React state with a one-way URL mirror. **Routes:** `/` → the Goals screen at the current lens; `/goals/:lens?period=<periodKey>`; `/goals/:id`; `/tasks/:id`; `/backlog`; `/learnings`. **Overlays** stay state (R-lens-14). Browser back and forward work, a deep link restores lens + period, and an unknown route lands on the Goals screen rather than a blank page. *Supersedes R-nav-2 and the "no router" decision it recorded — CR-5's task page is exactly the genuinely-linkable case that decision reserved.*
- **R-nav-25 (the top-right cluster)** — Unchanged in form (R-nav-11): the theme toggle plus **at most one** primary action per page. The mapping changes with the screens — Goals → `+ New goal` (created at the current lens's horizon, into the selected period); Goal detail → `+ Weekly goal` on a Monthly goal, `+ Task` on a Weekly goal, `+ Add` (backlog) on Yearly/Quarterly, none on Life; Backlog → `+ Add`; Task page → none; Learnings → none.
- **R-nav-26 (removed by design — extended)** — R-nav-14 is unchanged, and A2 adds to it: **a lens is not a report.** No weekly review wizard, no push flow with mandatory reasons, no audit-trail view, no week report, no carry-count flag — **and** no roll-up, completion rate, streak, burndown, or period summary on any lens header, and no completion state on a goal (R-goal-45). The only numbers in the product are R-lens-4's open count, R-goal-24's carrying line, `N in backlog`, and the carry chip. Any other number is out of scope and must be refused, not deferred.
...
- **S-lens-3-2** (R-lens-3, unhappy — a broken chain) — *Given* a goal whose `parentId` names a goal that does not exist. *Then* it renders in the `Unattached` group at the end of the lens; it is neither dropped from the view nor does it error.
...
- **S-lens-5-1** (R-lens-5, happy) — *Given* two Life goals and an orphan. *Then* the groups are ordered by the Life goals' `createdAt` asc, `id` asc, `Unattached` last; the order is identical on every read of an unchanged account.
...
- **S-lens-11-1** (R-lens-11, happy — badges) — *Given* the Weekly lens at `+1`. *Then* the badge reads `Planning ahead` and no task row renders a completion checkbox. *Given* `−1`. *Then* `Past — still editable`. *Given* the current week. *Then* neither. *And* the same three states hold in the Monthly, Quarterly and Yearly lenses for their own periods.
...
⚠ **A2.** `WeeklyFocus.sortKey` disappears with the entity. Goals gain **no** `sortKey`: within a lens group, items share one `periodKey` and are ordered `createdAt` asc, `id` asc (R-lens-5), and lens groups are ordered by their Life goal's `createdAt` asc, `id` asc with `Unattached` last (R-lens-3). The carried band in the Weekly lens is ordered by `periodKey` **ascending** — oldest first — which is the one place a period is a sort key rather than a filter (R-lens-12). `BacklogItem.sortKey` is unchanged.
...
**The consequential part is not the numbers, it is what they invalidate.** `domain/goal-tree.ts` states its own design premise as "at most 500 nodes, at most 4 levels deep … so nothing here needs a query", and every function in it takes the owner's **full goal list** and scans it: `isLeaf` is a `.some` over all goals, called from inside `orderedTree`'s walk. At 500 nodes that is free; at 10,000 it is O(n²) — around 10⁸ comparisons on a Worker, per request. **A2 therefore invalidates the in-memory-whole-tree design, not merely a constant.** Two rules already follow from this and are stated in §2: the lens read is scoped to one horizon and one period and never returns the whole tree (R-lens-16), and the server — not the client — resolves each item's Life root (R-lens-3). What remains for the build is to replace the full-list scans with period-scoped queries and an index on `(user_id, horizon, period_key)`, which does not exist today. **A build that raises the caps and leaves the scans is a build that gets slower every week and fails in its second year.**
...
**Q-20 — Should a Weekly goal be creatable from the Monthly lens?** `[recommended]` Yes, and it is the natural flow: a Monthly goal's row offers `+ Weekly goal`, which creates it into the **current** week by default (or, when the Monthly lens is showing a future month, the first week of that month). Requiring the owner to zoom to Weekly first would make the commonest create a two-lens journey.

**Q-21 — How large may the carried band grow before it needs a control?** R-lens-12 deliberately never ages anything out. `[recommended]` No control, and no cap — but the band is collapsible **as a whole** (one disclosure, remembered per session, never per goal), so a week with twelve carried goals still opens on this week's plan. That is presentation, not a rule, and it introduces no state on any row. If the band is routinely large the product is telling the truth about the account, and the answer is to finish or cancel work, not to hide it.

**Q-22 — Does `Repeat last week` copy across all Life lines or one?** `[recommended]` One line at a time, from the Life group's own header, because "repeat everything" on a busy account creates twenty goals in one tap with no review. An account-wide variant is a follow-up, not a v1.
...
| R-nav-5/17 | `Past week — still editable` / `Future week — planning ahead` | `Past — still editable` / `Planning ahead`, at every horizon (R-lens-11) |
...
- **Q-12 (collection sizes)** — **broken in three places and rewritten.** This is the cost the redesign does not pay for itself: a weekly intent is now a row in `goals`, an account grows by ~1,000 goals a year, and `domain/goal-tree.ts`'s whole-list, O(n²) design is premised on 500. R-lens-16 is the rule that follows; the query work is the build's.
