...
export const ideasRoutes = new Hono<AppBindings>()
  ...
  // `GET /backlog?goalId=` filters; this list does not, and now says so.
  .get(E.ideas, zQuery(NoQuery), async (c) => c.json(await c.get('container').resolve(IdeaService).list(ctx(c))))
